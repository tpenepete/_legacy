...
foreach($fields_data as $rf) {
    switch(strtolower($rf['control'])) {
        ...
        case 'sec' :
            ...
            if(preg_match('#^\{(.*)\}$#', $rf['value'], $mtch)) {
                ...
                include(TPT_TEMPLATES_DIR.DIRECTORY_SEPARATOR.$ifile);
                            ...
                            $section .= '<div class="float-left text-align-left padding-left-10 width-234">'.$rcontrols.'</div>';
                ...
                $section = '<div class="tpt_form_section'.$rf['classes'].'" '.$rf['html_attribs'].'>';
                    $section .= '<div>';
                        ...
                        if(!empty($fbl)) {
                            ...
                            $section_before_content .= '<div class="float-left text-align-left padding-left-10 width-234">'.$rcontrols_before.'</div>';
                        ...
                        $section .= '<div class="tpt_form_section_body overflow-hidden left-0 right-0'.$section_class.'"'.$secid.'>';
                            ...
                            $section .= '<div class="tpt_form_section_content"'.$secbid.'>';
                            $section .= ${$contvar};
                            $section .= '</div>';
                        $section .= '</div>';
                    $section .= '</div>';
        ...
        case 't' :
            ...
            $rcontrols .= tpt_html::createTextinput($tpt_vars, $rf['name'], $tpt_vars['template_data']['form_values'][$rf['name']], ' size="5" class="plain-input-field padding-top-4 padding-bottom-4 font-size-14" '.$rf['html_attribs']);
            $rcontrols .= '</div>';
            $rcontrols .= '</div>';
            $rcontrols .= '</div>';
            $rcontrols .= '</div>';
        ...
        case 'p' :
            ...
            $rcontrols .= '<div style="background-image: url('.$tpt_imagesurl.'/user-form-field-right.png);" class="padding-right-8 background-position-RC background-repeat-no-repeat">';
            $rcontrols .= '<div style="background-image: url('.$tpt_imagesurl.'/user-form-field-mid.png);" class="background-repeat-repeat-x background-position-CC">';
            $rcontrols .= tpt_html::createPasswordinput($tpt_vars, $rf['name'], '', ' size="5" class="plain-input-field padding-top-4 padding-bottom-4 font-size-14" '.$rf['html_attribs']);
            $rcontrols .= '</div>';
            $rcontrols .= '</div>';
            $rcontrols .= '</div>';
            $rcontrols .= '</div>';
