...
foreach($fields_data2 as $rf2) {
    switch(strtolower($rf2['control'])) {
        ...
        case 't' :
            ...
            $rcontrols2 .= tpt_html::createTextinput($tpt_vars, $rf2['name'], isset($tpt_vars['template_data']['form_values'][$rf2['name']])?$tpt_vars['template_data']['form_values'][$rf2['name']]:'', ' size="5" class="plain-input-field padding-left-2 padding-right-2 padding-top-2 padding-bottom-2 font-size-14" '.$rf2['html_attribs']);
            //$rcontrols2 .= '</div>';
            //$rcontrols2 .= '</div>';
            //$rcontrols2 .= '</div>';
            $rcontrols2 .= '</div>';
        ...
        case 'p' :
            ...
            //$rcontrols2 .= '<div style="background-image: url('.$tpt_imagesurl.'/user-form-field-right.png);" class="padding-right-8 background-position-RC background-repeat-no-repeat">';
            //$rcontrols2 .= '<div style="background-image: url('.$tpt_imagesurl.'/user-form-field-mid.png);" class="background-repeat-repeat-x background-position-CC">';
            $rcontrols2 .= tpt_html::createPasswordinput($tpt_vars, $rf2['name'], '', ' size="5" class="plain-input-field padding-left-2 padding-right-2 padding-top-2 padding-bottom-2 font-size-14" '.$rf2['html_attribs']);
            //$rcontrols2 .= '</div>';
            //$rcontrols2 .= '</div>';
            //$rcontrols2 .= '</div>';
            $rcontrols2 .= '</div>';
...
//            $select = $tpt_vars['modules']['handler']->modules[$rf2['value']]->{$rf2['name'].'Select'}($tpt_vars);
            ...
            break;
        case 'stsel' :
            $country = isset($tpt_vars['template_data']['form_values']['country'])?$tpt_vars['template_data']['form_values']['country']:'';
            $state = isset($tpt_vars['template_data']['form_values']['state'])?$tpt_vars['template_data']['form_values']['state']:'';
            $shipping = false;
            if($rf2['name'] == 'shipping_state') {
                $country = isset($tpt_vars['template_data']['form_values']['shipping_country'])?$tpt_vars['template_data']['form_values']['shipping_country']:'';
                $state = isset($tpt_vars['template_data']['form_values']['shipping_state'])?$tpt_vars['template_data']['form_values']['shipping_state']:'';
                $shipping = true;
            }
        ...
        default :
            ...
            $rcontrols2 .= '<div id="'.$rf2['name'].'_tptformcontrol'.'" class="urontrol height-'.$rf2['row_height'].' line-height-'.$rf2['control_line_height'].' padding-top-4 padding-bottom-4'.$rf2['classes'].'">'.tpt_html::createCheckbox($tpt_vars, $rf2['name'], $rf2['value'], $tpt_vars['template_data']['form_values'][$rf2['name']], $rf2['html_attribs'], $rf2['oncheck'], $rf2['onuncheck']);
            $rcontrols2 .= '</div>';
            $rafter2 .= '<div class="urontrol height-'.$rf2['row_height'].' line-height-'.$rf2['after_line_height'].' padding-top-4 padding-bottom-4 position-relative'.$rf2['classes'].'">';
            $rafter2 .= '<div class="position-relative" style="height: 100%; top: 50%;">';
            $rafter2 .= '<div class="position-relative" style="height: 100%; top: -50%;">';
...
$section_html .= '<div class="float-left text-align-left padding-left-10 width-234">'.$rcontrols2.'</div>';
