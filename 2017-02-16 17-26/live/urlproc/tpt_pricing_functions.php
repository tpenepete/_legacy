...
if(strtolower($_SERVER['REQUEST_METHOD']) == 'post') {
    ...
    */
    if((strtolower($task) == 'cart.add_stock')) {
        ...
        $fproducts = $fproducts + (isset(amz_cart::$pArray['stock'])?amz_cart::$pArray['stock']:array());
    ...
    if((strtolower($task) == 'cart.add_custom')) {
        
        amz_cart::$pricing_products = amz_cart::$products;
        
        //tpt_dump($tpt_vars['modules'], true);
        $products = amz_cart::processCustomProductPOSTData($tpt_vars, true);
        ...
        foreach($products as $key=>$product) {
            ...
            if(!empty($_POST['allpricing']) && ($_POST['allpricing'] == '4131')) {
                $bandSize = getModule($tpt_vars, 'BandSize')->moduleData['id'][$product->data['band_size']]['name'];
        ...
        foreach ($_POST['product'] as $p)
        {
            $style = $p['band_style'];
            $type = $p['band_type'];
			if(!empty(getModule($tpt_vars, 'BandData')->typeStyle[$type][$style])) {
				$cur_qty = $p['qty_sm'];
				...
				$product_pricing[$i]['price'] = (isset($tmp_prc_obj->price['html']['lowest_price_total_discounted'])?$tmp_prc_obj->price['html']['lowest_price_total_discounted']:'N/A');
				$product_pricing[$i]['per_band'] = (isset($tmp_prc_obj->price['html']['lowest_price_per_discounted'])?$tmp_prc_obj->price['html']['lowest_price_per_discounted']:'');
			} else {
				$product_pricing[$i]['pos'] = $i;
				$product_pricing[$i]['price'] = 'N/A';
				$product_pricing[$i]['per_band'] = '';
			}
            $i++;
        }
