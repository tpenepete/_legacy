...
EOT;
		/* AIK Debug */
		if ($_SERVER["REMOTE_ADDR"] == '89.253.191.44') {
			$template .= <<< EOT
<div id="aik_debug"></div>
EOT;
		}
		$template .= <<< EOT
        <script defer type="text/javascript" src="$js_url/tool-tip.js"></script>
        <div id="tpt_overlay" class="display-none position-fixed top-0 left-0 right-0 bottom-0 opacity-70" style="background-color:#000;z-index: 127000;" onclick="hide_lightbox();clear_tlcontent();"></div>
        <div id="tpt_lightbox_wrap" class="display-none position-fixed top-0 left-0 right-0 bottom-0 " style="width:150%;height:150%;left:50%;top:50%;z-index: 127001;">
        	<div id="tpt_lightbox" class="text-align-center" style="background-color: #f1ede9;width:50%;height:50%;position: relative;left:-25%;top:-25%;">
				...
				<div id="tpt_lightbox_content" class="overflow-hidden height-100prc" style="overflow:scroll;">
				</div>
			</div>
        </div>
				...
				<div class="outer-wrapper position-relative " id="amz_outwrap" style="z-index: 1; max-width: 100%;">
					...
					<div class="content clearFix text-align-center position-relative">
						...
						<div class="text-align-center" style="">
							<div class="amazing-discount display-inline-block width-90prc" style="border-radius: 5px 5px 5px 5px; border-color: #D72525; border-style: solid; border-width: 1px 1px 1px 1px;">
								<div class="amazing-discount height-53 line-height-53 text-align-center font-weight-bold" style="color: #F4E4CA; border-radius: 5px 5px 5px 5px; background: #D72525 none; border-color: #F4E4CA; border-style: solid; border-width: 1px 1px 1px 1px; font-family: Verdana, Tahoma, Helvetica, Arial, sans-serif;">
...
//$template = Minify_HTML::minify($template);
		...
		if ((isDev('rebuildcache') && !empty($_GET['rebuildcache']))) {
			tpt_template::rebuildCache($vars, $template, 'content_mobile1');
		}
