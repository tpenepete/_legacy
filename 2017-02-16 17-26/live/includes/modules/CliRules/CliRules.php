...
class tpt_module_CliRules extends tpt_Module {
	...
	function __construct(&$vars, $name, $moduleClassFile, $moduleClass, $moduleTable) {
		$fields = array(
			...
			new tpt_ModuleField('google_tag_manager',   'ti', '',   '',   '', 'tf', ' style="width: 230px;"', '0', 'Load the google tag manager code after <body>', false, false, 'LC'),
	...
	function parseRule(&$vars, /*$table, */$input/*, &$evars*/) {
		//$_tbl = $table;
		//tpt_dump($_tbl);
		//tpt_dump($table);
		//tpt_dump($url, true);
...
(
    ...
    WHERE
        `enabled`=1
        AND
        "$input" REGEXP REPLACE(TRIM(REPLACE(`url_preg_pattern`, "#", " ")), " i", "")
        AND
        `is_ajax`=0
        AND
        `process_url`=0
...
EOT;
		...
		//tpt_dump($query);
		//tpt_dump($murls, true);
		tpt_logger::dump($vars, $input, debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS), 'URL', __FILE__.' '.__LINE__);
