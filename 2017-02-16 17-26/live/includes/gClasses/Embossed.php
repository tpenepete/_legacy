...
class tpt_gclass_Embossed extends tpt_PreviewGenerator {
	...
	if($options['swirlColor'] !== false) {
	    $IMCommand = BIN_PATH.IMAGEMAGICK_BIN.' ';
	    $IMCommand .= '	'.TPT_CACHE_DIR.DIRECTORY_SEPARATOR.'1_2'.DIRECTORY_SEPARATOR.'swirl.png ';
	    $IMCommand .= '	png:- ';
	...
	$IMCommand .= '	-alpha Set ';
	$IMCommand .= '	png:- ';
        $this->exec($vars, $steps, 'BandNoText', $IMCommand, 3, $steps['TextMaskDistortExtent'].$steps['Band']);
	...
	if($options['swirlColor'] !== false) {
	    ...
	    $IMCommand .= '	png:- ';
	    $IMCommand .= '	png:- ';
	    $this->exec($vars, $steps, 'BandNoText', $IMCommand, 3, $steps['BandNoText'].$steps['Swirl']);
	    //return $steps['BandNoText'];
	}
