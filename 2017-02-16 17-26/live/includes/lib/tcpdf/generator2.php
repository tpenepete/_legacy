...
//$pdf->setPageFormat(array(279.4,215.9),'L');
...
EOT;
	...
	static function add_page() {
		...
		$pdf->AddPage('L',array(279.4,215.9));

		require(dfls.'template-customer2.php');
		...
		$pdf->SetFont('helveticaltstdcompressed', '', 12, '', true);
		$pdf->SetTextColor(114,134,132);
		$pdf->setFontSpacing(0.2);
		
		$pdf->startTransaction();
		$pdf->setCellHeightRatio(1);
		$pdf->MultiCell(56,10,$t.'-1',0,'L',0,1,$ax,$ay);
		$y = $pdf->getY();
		if ($y<50) {
			$pdf->commitTransaction();
		} else {
			$pdf->SetFont('helveticaltstdcompressed', '', 11, '', true);
			$pdf->rollbackTransaction(true);
			$pdf->startTransaction();
			$pdf->setY($y);
			$pdf->setCellHeightRatio(0.86);
			$pdf->MultiCell(56,10,$t.'-2',0,'L',0,1,$ax,$ay);
			$y = $pdf->getY();
			...
			//	var_dump('ssasa');
				...
				$pdf->MultiCell(56,10,$t.'-3',0,'L',0,1,$ax,$ay);
				$y = $pdf->getY();
			}
		...
		$t.= "</b>";
		$pdf->writeHTMLCell(0,10,145,26,$t);
		
		$pdf->SetFont('helveticaltstdcompressed', '', 14, '', true);
		$t = "<b color=\"#D92929\" style=\"line-height:12.4pt; text-align:left;\">";
		$t.= 'Message Style:';
		$t.= "</b>";
		$pdf->writeHTMLCell(0,10,85,42,$t);
		...
		foreach ($ord_data as $ord) {
			...
			//	echo '</pre>';
				...
				if (self::v('p')!=self::v('p1')) {
					if ($rush) {
						$pdf->ImageSVG(dfls.'img/rush2.svg',150,15);
						$pdf->ImageSVG(dfls.'img/paid5.svg',128,15);	
					} else {
